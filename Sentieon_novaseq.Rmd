...
```{r}
library(tidyr)
mydir = "~/Documents/Projects/myeloid/1.9.0/sentieon/novaseq/qualimap_bam2.1.0/"
total_files =  list.files(path = paste0(mydir), pattern= ("*.txt"))

summary_df  <- as.data.frame(matrix(ncol = length(total_files), nrow = 58))
summary_df_v2 <- as.data.frame(matrix(ncol = 1, nrow = 37))
rownames <- c()
for(sample in 1:length(total_files)) { #up to 4 samples
  print(sample)
  file = list.files(path = paste0(mydir), pattern= ("*.txt"))[sample]
  print(file)
  dat <- as.data.frame(readLines(paste0(mydir, file)))
  # split the fourth line that contains the input file by / then change vector and select the last element which is the sample name. This will be the rowname for this file
  sample_name <-  tail(unlist(strsplit(dat[6,], "/")),1)
  colnames(summary_df)[sample] <- sample_name
  summary_df[,sample] <- dat[c(16:73),]
  #remove NA's or blank rows
  summary_df2 <- summary_df[!apply(is.na(summary_df) | summary_df == "", 1, all),]
  summary_df3 <- summary_df2 %>% 
   separate(sample_name, c("headers", sample_name), sep = "=")
  row_names <- summary_df3[,"headers"]
  summary_df_v2 <- as.data.frame(cbind(summary_df_v2, summary_df3[,sample_name]))
  colnames(summary_df_v2)[sample+1] <- sample_name
}

rownames(summary_df_v2) <- row_names
summary_df_v2 <- summary_df_v2[,-1]
# remove bam and replace with v2
colnames(summary_df_v2) <-paste0(unlist(strsplit(colnames(summary_df_v2), ".bam")), "_v2")
```

```{r}
mydir = "~/Documents/Projects/myeloid/1.9.0/sentieon/novaseq/qualimap_bam3.2.0/"
total_files =  list.files(path = paste0(mydir), pattern= ("*.txt"))

summary_df  <- as.data.frame(matrix(ncol = length(total_files), nrow = 58))
summary_df_v3<- as.data.frame(matrix(ncol = 1, nrow = 37))
rownames <- c()
for(sample in 1:length(total_files)) { #up to 4 samples
  print(sample)
  file = list.files(path = paste0(mydir), pattern= ("*.txt"))[sample]
  ...
  summary_df3 <- summary_df2 %>% 
   separate(sample_name, c("headers", sample_name), sep = "=")
  row_names <- summary_df3[,"headers"]
  summary_df_v3 <- as.data.frame(cbind(summary_df_v3, summary_df3[,sample_name]))
  colnames(summary_df_v3)[sample+1] <- sample_name
}

rownames(summary_df_v3) <- row_names
summary_df_v3 <- summary_df_v3[,-1]
# remove bam and replace with v2
colnames(summary_df_v3) <-paste0(unlist(strsplit(colnames(summary_df_v3), ".bam")), "_v3")
```

```{r}
# Now we bind and comapre the alignment of v2 and v3 :)

bam_df_v2_v3 <- cbind(summary_df_v2, summary_df_v3)
bam_df_v2_v3 <- bam_df_v2_v3[,order(colnames(bam_df_v2_v3))]

# write.table(bam_df_v2_v3, "novaseq/qualimap_results.tsv", sep = "\t", quote = F, row.names = rownames(bam_df_v2_v3))
```
